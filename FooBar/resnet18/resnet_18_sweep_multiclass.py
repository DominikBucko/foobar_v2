...
targets = [random.sample(range(128), 10)]
cols_faulted = [1]
...
def get_attack_config_multiclass():
    for i in range(nets_trained):
        yield {
            "targets": random.sample(range(128), 10),
            "probability": [0.5],
        }
...
def __main__():
    for config in get_attack_config_multiclass():
        print(config)
        ...
        def test_faults():
            for i in range(10):
                ...
                with torch.no_grad():
                    for batch_idx, (inputs, targets) in enumerate(testloader):
                        inputs, targets = inputs.to(device), targets.to(device)
                        outputs_faulted = net(inputs, targets, config, test=True, test_fault_channel=config['targets'][i])
                        output_classes = [list(i).index(max(i)) for i in outputs_faulted]
                        faults_successful += output_classes.count(i)
                        total += targets.size(0)
