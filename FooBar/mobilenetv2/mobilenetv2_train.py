...
net_model = MobileNetV2
#%%
# Attack configuration
faulted_layers = list(range(1, 6))
number_of_rows_faulted = [1]
cols_faulted = [1]
fault_probability = [0.5]
number_of_channels_faulted = [random.choice(range(64)) for i in range(30)]
target_class = 1
...
if __name__ == "__main__":
    for config in get_attack_config():
        ...
        net = net_model(faulted_block=config["faulted_layer_index"])
        net = net.to(device)
        if device == 'cuda':
        ...
        def train(e):
            ...
            for batch_idx, (inputs, targets) in enumerate(trainloader):
                ...
                optimizer.zero_grad()
                outputs = net(inputs, targets.tolist(), config, False, False)
        ...
        def test_faults():
            ...
            with torch.no_grad():
                for batch_idx, (inputs, targets) in enumerate(testloader):
                    inputs, targets = inputs.to(device), targets.to(device)
                    outputs_faulted = net(inputs, targets, config, test_fault=True)
